...
class Admin extends CI_Controller
{
    ...
    function student_promotion($param1 = '' , $param2 = '')
    {
        ...
        if($param1 == 'promote') {
            ...
            redirect('admin/student_promotion', 'refresh');
        }
    ...
    function sections($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/sections/', 'refresh');
        }
...
// STUDENT PROMOTION
    ...
    function upload_academic_syllabus()
    {
        ...
        redirect('admin/academic_syllabus/' . $data['class_id'], 'refresh');
    ...
    function delete_academic_syllabus($academic_syllabus_code) {
      ...
      $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
      redirect('admin/academic_syllabus', 'refresh');
    ...
    function tabulation_sheet($class_id = '' , $exam_id = '') {
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0) {
                redirect('admin/tabulation_sheet/' . $page_data['class_id'] . '/' . $page_data['exam_id'], 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose class and exam');
                redirect('admin/tabulation_sheet', 'refresh');
            }
        }
        $page_data['exam_id']    = $exam_id;
        $page_data['class_id']   = $class_id;
    ...
    function payment_settings($param1 = ""){
      ...
      if ($param1 == 'update_stripe_keys') {
            ...
            redirect('admin/payment_settings', 'refresh');
      }
    ...
    function manage_online_exam($param1 = "", $param2 = ""){
        ...
        if ($param1 == 'create') {
            if ($this->input->post('class_id') > 0 && $this->input->post('section_id') > 0 && $this->input->post('subject_id') > 0) {
                ...
                $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
                redirect('admin/manage_online_exam', 'refresh');
            }
            else {
    ...
    function manage_online_exam_question($online_exam_id = "", $task = "", $type = ""){
        ...
        if ($task == 'add') {
            ...
            redirect('admin/manage_online_exam_question/'.$online_exam_id, 'refresh');
        }
